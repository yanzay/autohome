...
func NewSqliteStorage(params ...string) *sqliteStorage {
...
func (s *sqliteStorage) CreateTable(obj interface{}) error {
	query := s.CreateTableSQL(obj)
	err := s.db.Query(query).Run()
	return err
}

func (s *sqliteStorage) Create(obj interface{}) error {
	fmt.Printf("Create in sqlite_storage %q\n", obj)
	query := s.InsertSQL(obj)
	fmt.Printf("Query: %v\n", query)
	err := s.db.Query(query).Run()
	if err != nil {
		fmt.Printf("Fail to save %v, error: %v\n", obj, err)
	}
	return err
}

func (s *sqliteStorage) All(obj interface{}) jet.Runnable {
	query := s.SelectAllSQL(obj)
	items := s.db.Query(query)
	return items
}
